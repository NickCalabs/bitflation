...
async function fetchChunk(toTs, limit = 2000) {
  const url = `https://min-api.cryptocompare.com/data/v2/histoday?fsym=BTC&tsym=EUR&limit=${limit}&toTs=${toTs}`;
...
async function main() {
  console.log('Fetching BTC/EUR daily price data from CryptoCompare...');
  ...
  console.log(`\nWrote ${entries.length} entries to src/data/eur/btc-daily.json (${sizeKB}KB)`);
}
