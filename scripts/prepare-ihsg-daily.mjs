...
async function main() {
  ...
  if (!res.ok) {
    console.error(`Yahoo Finance HTTP ${res.status}: ${res.statusText}`);
    console.error(
      "Try: .venv/bin/python scripts/prepare-ihsg-daily-yfinance.py (pip install -r scripts/requirements-data.txt)",
    );
