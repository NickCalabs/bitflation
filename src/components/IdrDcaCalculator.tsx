...
// IHSG data starts 2015; max ~11 years back from today
...
export function IdrDcaCalculator() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      import('../data/idr/cpi-monthly.json').then((m) => m.default as { date: string; value: number }[]),
    ])
  ...
  const chartData = useMemo(() => {
    ...
    // Downsample for display when there are many points (e.g. daily over 11 years)
    const maxPoints = 600;
  ...
  // IDR CPI inflation over the selected period (for context)
  const inflationOverPeriod = useMemo((): { startYear: number; pct: number } | null => {
    ...
    const startStr = start.toISOString().slice(0, 7); // YYYY-MM
    ...
    return (
      <section className={styles.section}>
        <div className={styles.loading}>
          Data not available. Run: <code>node scripts/prepare-ihsg-daily-yfinance.py</code> and <code>node scripts/prepare-gold-idr-daily.mjs</code>
        </div>
      </section>
    );
  }
